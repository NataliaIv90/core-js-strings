...
function removeFirstOccurrences(str, value) {
  ...
  // Use string slicing to remove the last occurrence of the substring
...
function reverseWords(str) {
  ...
  words.forEach((el, index) => {
    words[index] = el.split('').reverse().join('');
  });
  return words.join(' ');
  // throw new Error('Not implemented');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (/[A-Za-z]/.test(char)) {
      ...
      encodedStr += isUpperCase ? shiftedChar : shiftedChar.toLowerCase();
    } else {
      encodedStr += char;
    }
  }
